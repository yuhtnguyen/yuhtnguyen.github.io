...
import { DownloadOutlined, FileTextOutlined, MailOutlined, PhoneOutlined } from '@ant-design/icons'
  ...
  const education = [
    {
      ...
      period: '2018 - 2022',
      description: 'Graduated with honors. Relevant coursework: Data Structures, Algorithms, Database Systems, Software Engineering'
    }
  ]
  ...
  return (
    <div className="page-container">
      <div style={{ textAlign: 'center', marginBottom: '48px' }}>
        <Title level={2} className="section-title">Resume</Title>
      ...
      <Row gutter={[32, 32]}>
        ...
        <Col xs={24} lg={8}>
          <Card title="Contact Information" style={{ marginBottom: '24px' }}>
            <Space direction="vertical" size="middle">
              <div>
                <MailOutlined style={{ marginRight: '8px', color: '#1677ff' }} />
        ...
        <Col xs={24} lg={16}>
          <Title level={3} style={{ color: '#1677ff' }}>Work Experience</Title>
          {experiences.map((exp, index) => (
            <Card key={index} style={{ marginBottom: '24px' }}>
              <div style={{ marginBottom: '16px' }}>
                <Title level={4} style={{ marginBottom: '4px' }}>{exp.title}</Title>
                <Text strong style={{ color: '#1677ff' }}>{exp.company}</Text>
                <Text style={{ float: 'right', color: '#666' }}>{exp.period}</Text>
              </div>
          ...
          <Title level={3} style={{ color: '#1677ff' }}>Education</Title>
          {education.map((edu, index) => (
            <Card key={index} style={{ marginBottom: '24px' }}>
              <div style={{ marginBottom: '16px' }}>
                <Title level={4} style={{ marginBottom: '4px' }}>{edu.degree}</Title>
                <Text strong style={{ color: '#1677ff' }}>{edu.school}</Text>
                <Text style={{ float: 'right', color: '#666' }}>{edu.period}</Text>
              </div>
